...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside 
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-zinc-950 text-white flex flex-col h-full shadow-xl md:shadow-lg transform transition-transform duration-300 ease-in-out ${
          isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}
      >
        {/* Header Logo & School Name */}
        <div className="py-3.5 px-3.5 border-b border-zinc-800 flex items-center justify-between bg-zinc-950">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center space-x-3 px-3.5 py-3 rounded-xl transition-all ${
                isActive 
                  ? 'bg-zinc-800 text-white font-medium shadow-sm shadow-black/50' 
          ...
          {isAdmin && (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center space-x-3 px-3.5 py-3 rounded-xl transition-all ${
                  isActive 
                    ? 'bg-zinc-800 text-white font-medium shadow-sm shadow-black/50 ring-1 ring-zinc-700' 
                    : 'text-zinc-400 hover:bg-zinc-800 hover:text-white'
                }`
              }
            >
              ...
              <div className="flex items-center justify-between flex-1">
                ...
                <span className="text-[9px] bg-amber-400 text-zinc-900 px-1.5 py-0.5 rounded font-black uppercase">
                  Admin
                </span>
              </div>
            </NavLink>
        ...
        {/* Sidebar Footer */}
        <div className="p-3.5 border-t border-zinc-800 text-center text-xs text-zinc-400 flex items-center justify-center space-x-2 bg-zinc-950">
